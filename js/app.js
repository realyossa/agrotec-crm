...
// pagina do site como LINK: o time clica e cai na pagina que trouxe o cliente (pedido do Yossa, 03/09)
const linkPag = (pg) => pg ? `<a class="lk-pag" href="https://agrotecimobiliarianoagro.com.br${h(pg)}" target="_blank" rel="noopener" title="Abrir a página do site">${h(pg)}</a>` : '';
...
function cartaoFila(n) {
  ...
  return `<article class="lead ${n.sem_contato ? cls : ''}" data-id="${n.id}" data-pessoa="${n.pessoa_id}" data-tel="${h(n.telefone || '')}" data-nome="${h(n.pessoa_nome || '')}">
    ...
    <div><div class="nome">${h(nomeOk(n.pessoa_nome))} <span class="codigo mono" style="color:var(--ouro)">${h(n.codigo || '')}</span> ${n.esteve_no_site ? `<span class="chip c-azul" title="Tem histórico de navegação antes de se identificar">${ICO.site} esteve no site</span>` : ''}</div>
      <div class="linha">${chipFunil(n.funil)} ${n.tipo ? `<span class="chip c-neutro">${h(n.tipo)}</span>` : ''} ${chipOrigem(n.origem_conversao)} ${n.pessoa_cidade ? `<span>${h(n.pessoa_cidade)}</span>` : n.cidade_ip ? `<span title="pelo IP, aproximado">~${h(n.cidade_ip)}</span>` : ''} <span>· ${h(n.origem_rotulo || n.origem_evento || '')} em ${linkPag(n.origem_pagina)}</span> ${n.corretor_nome ? `<span>· ${h(n.corretor_nome.split(' ')[0])}</span>` : ''} ${chipEtapa(n.etapa)}</div></div>
...
async function telaPessoas(q = '') {
  ...
  moldura('pessoas', `${topo('Pessoas', 'Uma linha por pessoa — o histórico junta todos os cliques dela', recorteHtml())}
    ...
    ${lista.map(p => { const n = (p.negocios || [])[0] || {}; return `<tr><td class="quando"><b>${fmtData(p.criado_em)}</b><small>${p.visitas > 1 ? p.visitas + ' visitas' : '1ª visita'}${p.esteve_no_site ? ' · esteve no site' : ''}</small></td>
      ...
      <td>${chipOrigem(p.origem_conversao)}</td><td><small>${h(n.origem_rotulo || n.origem_evento || '')}<br>${linkPag(n.origem_pagina)}</small></td>
  ...
  moldura('pessoas', `${topo(nomeOk(p.nome), `${p.codigo || ''}${p.interna ? ' · PESSOA INTERNA (sem cartões, fora da fila e dos números)' : ''} · entrou ${fmtData(p.criado_em)}${p.esteve_no_site ? ' · esteve no site antes de se identificar' : ''}`, `<button class="btn" id="interna" title="${p.interna ? 'Volta a gerar cartões' : 'Sócio, corretor ou dono: sem cartões, fora da fila e dos números'}">${p.interna ? 'Deixar de ser interno' : 'Marcar como interno'}</button><a class="btn" href="#/pessoas">← Pessoas</a>`)}
    <div class="pessoa-grade">
      <div>
        <div class="cx" style="margin-bottom:14px"><div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;align-items:center"><h2>Negócios</h2><button class="btn btn-p" id="novo-negocio">+ Negócio</button></div>
          ${d.negocios.map(n => `<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;padding:10px 0;border-top:1px solid var(--linha)" data-neg="${n.id}">${chipFunil(n.funil)}${chipEtapa(n.etapa)}${n.tipo ? `<span class="chip c-neutro">${h(n.tipo)}</span>` : ''}
            <span style="font-size:13px;color:var(--tinta-2)">${h(n.origem_rotulo || n.origem_evento || '')} em ${linkPag(n.origem_pagina)} · ${fmtData(n.criado_em)}</span>
      ...
      <div class="cx"><h2>Dados</h2><dl class="dados">
        ...
        <dt>Primeira visita</dt><dd class="chips">${chipOrigem(p.origem_primeira)}${p.origem_primeira?.landing ? `<a class="chip c-borda lk-pag" href="https://agrotecimobiliarianoagro.com.br${h(p.origem_primeira.landing)}" target="_blank" rel="noopener">${h(p.origem_primeira.landing)}</a>` : ''}</dd>
...
async function telaIA() {
  ...
  moldura('ia', `${topo('IA lendo o site', 'Quem é máquina, o que leu e o que isso virou · 28 dias' + (T.medindo_desde ? ` · robôs medidos desde ${fmtDia(T.medindo_desde)}` : ''))}
    ...
    ${(d.protocolo || []).length ? `<div class="cx" style="margin-bottom:14px"><h2>Consultas de permissão</h2><div class="sub">Antes de ler, o robô consulta o <span class="mono">robots.txt</span> para saber se pode, e o <span class="mono">sitemap.xml</span>/<span class="mono">llms.txt</span> para saber o que existe. Não é leitura de conteúdo, por isso fica fora da tabela acima.</div>
      ${cab(['Arquivo', 'Robô', 'Vezes', 'Tipo', '', ''])}
      ${d.protocolo.map(x => `<div class="funil-ev"><div class="mono">${h(x.caminho)}</div><div><b>${h(x.agente)}</b></div><div class="n num">${x.n}</div><div>${chipFin(x.finalidade)}</div><div></div><div></div></div>`).join('')}</div>` : ''}
